...
const ProductCard = ({ data, cart, like }) => {
  ...
  const { addToLike, removeFromLike } = useContext(UseMainContext)
  return (
    <div className="relative h-[403px] border-2 border-gray">
      ...
      <button onClick={() => (like ? removeFromLike(data.id) : addToLike(data))} className= "absolute top-4 right-4">
        <Icons.likeIcon color={like && 'red'} />
      </button>
      <button onClick={() => (cart ? removeFromCart(data.id) : addToCart(data))} className={clsx(`${cart ? "border border-primary" : "border-none bg-primary"} absolute bottom-0 right-0 py-2 px-4 rounded-tl-[10px] border border-primary `)}>
